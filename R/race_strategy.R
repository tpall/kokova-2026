...
md <- c(md, "",
  ...
  "|---------|---------|----------|",
  sprintf("| HRV nädala keskmine | %d | 22.–24.07 seis — üle enda 2026. aasta keskmise (59,8) |", ATHLETE$hrv_recent),
  sprintf("| Kroonilne koormus | %d | **%.0f%% aprilli tipust (%d)** — 24.07 seis, enne luureplokki |",
          ATHLETE$chronic_load, 100 * ATHLETE$chronic_load / ATHLETE$chronic_peak,
          ATHLETE$chronic_peak),
  sprintf("| Pikim sõit pärast TBR-i | %.0f km | 30. juuli: luure + laagrisõit ühe päevaga, täiskoormaga |",
          ATHLETE$longest_ride_since_race_km),
  sprintf("| Luureplokk 30.–31.07 | ~%.0f km | Kaks järjestikust koormatud päeva (%.0f + ~%.0f km) |",
          ATHLETE$recon$day1_total_km + ATHLETE$recon$day2_km,
          ATHLETE$recon$day1_total_km, ATHLETE$recon$day2_km),
  "",
  ...
  sprintf("Teine pool: Garmin küsis 225 km päeva järel **%d h taastumist** — sisuliselt skaala lagi. Tipptempo on alles, aga suure päeva hind on madala kroonilise koormuse pealt praegu maksimumilähedane.",
          ATHLETE$recon$recovery_req_h),
